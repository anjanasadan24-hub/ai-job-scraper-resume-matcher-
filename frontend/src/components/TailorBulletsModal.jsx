...
export default function TailorBulletsModal({ job, onClose }) {
  ...
  useEffect(() => {
    ...
    async function fetchBullets() {
      ...
      setError(null);
      try {
        const res = await api.tailorBullets(job.id);
        setBullets(res.content);
        setSource(res.source);
      } catch (err) {
        setError(err.message || 'Failed to generate tailored bullets');
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl flex flex-col shadow-2xl overflow-hidden my-8">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-slate-800 bg-slate-850">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-white">AI-Optimized Resume Bullets</h3>
              <p className="text-xs text-slate-400">Tailored for {job.title} at {job.company}</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          {loading && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-xs text-slate-300 font-medium">Drafting ATS-optimized accomplishment bullets...</p>
            </div>
          )}
